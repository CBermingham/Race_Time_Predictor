...
# r = requests.get('http://www.runbritainrankings.com/rankings/rankinglist.aspx?event=5K&agegroup=V35&sex=M&year=2016')
...
# F = open('runbritain_athlete_data.txt','w+') 
# F.write(pretty_data.encode('utf-8'))
# F.close()

# --------------------------------------------

f = open('runbritain_athlete_data.txt', 'rU')
lines=f.readlines()
f.close()
...
writer = csv.writer(open("runbritain_data_2016_men_5K_V35.csv", "wb"))
writer.writerow(['athleteid'] + possible_events)
...
for link in links:
	...
	results = [ids[count]] + results

	# # Save data as a csv file
	writer = csv.writer(open("runbritain_data_2016_men_5K_V35.csv", "a"))
